...
//   logic cnt_up_sig;
//   logic cnt_down_sig;
//   logic overflow_state_from_counter;
//   logic [CounterWidth-1:0] in_flight_count;
//   logic sbr_port_rready_internal;

//   select_t select_d, select_q;

//   logic grant_condition_is_met_for_read; // Only for reads
//   logic grant_condition_was_met_q;    // Tracks previous grant_condition_is_met_for_read
...
//     // MODIFIED Forwarding logic (from Attempt 23):
//     // Both reads and writes use this more conservative condition.
//     sub_conditions_for_forwarding = target_is_current_selection || no_transactions_in_flight || one_transaction_is_completing;
    
//     // We ignore overflow_state_from_counter for forwarding decision
//     if (sbr_port_req_i.req && sub_conditions_for_forwarding) begin
//       mgr_ports_req_o[sbr_port_select_i].req = sbr_port_req_i.req;
//       mgr_ports_req_o[sbr_port_select_i].a   = sbr_port_req_i.a;
//       forward_this_request = 1'b1;
//     end

//     // Grant condition for counting (cnt_up_sig) is now only for READS that are forwarded and granted
...
//         end
//         if (forward_this_request) begin // Check if request is actually forwarded
//           // lint_off MEMBERNOTFOUND
...
//     .STICKY_OVERFLOW ( 1'b0         )
//   ) i_counter (
//     .clk_i, .rst_ni, .clear_i(1'b0),
//     .en_i      ( cnt_up_sig || (cnt_down_sig && (in_flight_count != {CounterWidth{1'b0}})) ),
//     .load_i(1'b0), .down_i(cnt_down_sig), .delta_i({{CounterWidth-1{1'b0}}, 1'b1}), .d_i('0),
//     .q_o(in_flight_count), .overflow_o(overflow_state_from_counter)
//   );
...
//   // --- Debug Prints (Concise) ---
//   logic overflow_state_prev_dbg;
//   always_ff @(posedge clk_i or negedge rst_ni) begin
//       if (!rst_ni) overflow_state_prev_dbg <= 1'b0;
//       else overflow_state_prev_dbg <= overflow_state_from_counter;
//   end

//   always @(posedge clk_i) begin
//     if (!rst_ni) begin
//     end else begin
//         if (overflow_state_from_counter && !overflow_state_prev_dbg) begin
//             $display("!!! %t [DEMUX_OVERFLOW_EVENT] Counter's overflow_o became 1! sel_q=%d, flight_cnt(prev)=%d, cnt_up_s=%b, cnt_down_s=%b !!!",
//                      $time, select_q, in_flight_count, cnt_up_sig, cnt_down_sig);
//         end
//         // if (sbr_port_req_i.req) begin 
//         //     $display("--- %t [DEMUX_TRACE_A23] ---", $time); 
//         //     $display("  In: req=%b, sel=%d, addr=0x%h, we=%b", sbr_port_req_i.req, sbr_port_select_i, sbr_port_req_i.a.addr, sbr_port_req_i.a.we);
//         //     $display("  State: sel_q=%d->sel_d=%d, flight_cnt=%d, ovf_ctr=%b", select_q, select_d, in_flight_count, overflow_state_from_counter);
//         //     $display("  Ctrl: up_s=%b, down_s=%b, grant_met_rd=%b, grant_was_met_q=%b", cnt_up_sig, cnt_down_sig, grant_condition_is_met_for_read, grant_condition_was_met_q);
//         //     $display("  Fwd: fwd_this_req=%b to mgr_port[%d] (actual_fwd_val=%b)", forward_this_request, sbr_port_select_i, mgr_ports_req_o[sbr_port_select_i].req);
//         //     $display("  Out: gnt=%b, rvalid=%b", sbr_port_rsp_o.gnt, sbr_port_rsp_o.rvalid);
//         //     $display("------------------------------------");
//         // end
//     end
//   end
...
// // this one worked with user domain tb but stall in helloworld
// import obi_pkg::*;      // For obi_pkg::ObiDefaultConfig
// import cf_math_pkg::*;  // For cf_math_pkg::idx_width
...
//     // Forwarding logic:
//     // Writes are less restricted as they don't increment the counter.
//     // Reads need to check if the channel is clear or completing.
//     if (sbr_port_req_i.a.we) begin // It's a WRITE request
//         sub_conditions_for_forwarding = 1'b1; 
...
//                      $time, select_q, in_flight_count, cnt_up_sig, cnt_down_sig);
//         end
//         // if (sbr_port_req_i.req) begin // Only print full trace if there's an incoming request
//         //     $display("--- %t [DEMUX_TRACE_A10] ---", $time); // Changed trace name
//         //     $display("  In: req=%b, sel=%d, addr=0x%h, we=%b", sbr_port_req_i.req, sbr_port_select_i, sbr_port_req_i.a.addr, sbr_port_req_i.a.we);
//         //     $display("  State: sel_q=%d->sel_d=%d, flight_cnt=%d, ovf_ctr=%b", select_q, select_d, in_flight_count, overflow_state_from_counter);
//         //     $display("  Ctrl: up_s=%b, down_s=%b, grant_met_rd=%b, grant_was_met_q=%b", cnt_up_sig, cnt_down_sig, grant_condition_is_met_for_read, grant_condition_was_met_q);
//         //     $display("  Fwd: fwd_this_req=%b to mgr_port[%d] (actual_fwd_val=%b)", forward_this_request, sbr_port_select_i, mgr_ports_req_o[sbr_port_select_i].req);
