module tb_obi_spi_peripheral;
  ...
  // Clock & reset
  logic clk, rst_n;
  initial clk = 0; always #5 clk = ~clk;  // 100 MHz-ish
  initial begin
    rst_n = 0; #20; rst_n = 1;            // hold reset for 20ns
  end
  ...
  obi_spi_peripheral dut (
    ...
    .be_i     (be_i),     .addr_i   (addr_i),
    .wdata_i  (wdata_i),
    .gnt_o    (gnt_o),    .rvalid_o (rvalid_o),
    .rdata_o  (rdata_o),
    .sck_o    (),        // no need to observe here
    .mosi_o   (), 
    .cs_no    ()
  );
  ...
  initial begin
    ...
    // 1) Write TX register (offset 0x004, one byte)
    req_i    = 1; we_i    = 1;
    be_i     = 4'b0001;
    addr_i   = 32'h00000004;
    wdata_i  = 32'h000000AA;  // arbitrary pattern
    $display("[%0t] TB: Issued WRITE TX", $time);
    wait (gnt_o) begin
      $display("[%0t] TB: WRITE TX granted", $time);
    end
    @(posedge clk); req_i = 0;

    // 2) Write CTRL register (offset 0x000) to start SPI
    @(posedge clk);
    req_i   = 1; we_i   = 1;
    be_i    = 4'b0001;
    addr_i  = 32'h00000000;
    wdata_i = 32'h1;
    $display("[%0t] TB: Issued WRITE CTRL", $time);
    wait (gnt_o) begin
      $display("[%0t] TB: WRITE CTRL granted", $time);
    end
    @(posedge clk); req_i = 0;

    // 3) Read STATUS register (offset 0x008)
    repeat (2) @(posedge clk); // give peripheral a couple cycles
    @(posedge clk);
    req_i   = 1; we_i   = 0;
    be_i    = 4'b1111;
    addr_i  = 32'h00000008;
    $display("[%0t] TB: Issued READ STATUS", $time);
    wait (gnt_o) begin
      $display("[%0t] TB: READ STATUS granted", $time);
    end
    @(posedge clk); req_i = 0;

    // 4) Wait for rvalid and check data
    wait (rvalid_o) begin
      $display("[%0t] TB: rvalid -> rdata=0x%08h", $time, rdata_o);
      if ( (rdata_o & 1)==0 )
        $error("STATUS busy bit never cleared!");
    end

    $display("[%0t] TB: Mini test complete, PASS", $time);
    $finish;
  end
